...
(async () => {
  try {
    console.log('🗑️ DELETANDO TODAS AS NOTIFICAÇÕES ANTIGAS...\n');
    
    const [result] = await pool.query(
      'DELETE FROM notificacoes_enviadas_jogos WHERE 1=1'
    );
    console.log(`✅ Deletadas ${result.affectedRows} notificações\n`);
    
    console.log('📋 REENVIANDO AGENDAMENTO...\n');
    ...
    console.log(`📊 Total de ${jogos.length} jogos`);
    ...
    conexao.release();
    console.log(`✅ Agendadas ${totalAgendadas} notificações com horários corrigidos!\n`);
    
    console.log('✨ Pronto! Execute o validador para confirmar:');
    console.log('   node validar-alertas-jogos.js');
