-- Script: inserir palpites + criar cobranças PIX (rodada 20, campeonato_id 69, users 1..6)
-- DB: bolaovip
-- BACKUP RECOMENDADO: mysqldump -u <user> -p bolaovip > backup_before_inserts.sql

-- Use the target database explicitly
USE `bolaovip`;

START TRANSACTION;

-- CONFIGURAÇÃO (altere se quiser)
SET @rodada = 20;
SET @campeonato_id = 69;
SET @grupo_id = 2;
SET @valor_palpite = 15.00;
-- Lista de usuários (ajuste conforme necessário)
SET @users_csv = '1,2,3,4,5,6';
...
-- 1) (OPTIONAL) Deletar palpites antigos e cobranças vinculadas (force = true behaviour).
-- Atenção: caso queira manter históricos, comente os DELETEs abaixo.
-- Deleta cobranças que referenciam palpites antigos desta rodada e usuários (payload.origim='palpites')
DELETE pc
...
-- Deleta palpites antigos para esses usuários/rodada/campeonato
DELETE p
FROM palpites p
...
-- 2) Inserir novos palpites (gols = 0-0 por determinismo)
-- Cada combinação (usuario x jogo) recebe 1 palpite
...
-- 3) Criar PIX cobrancas PENDENTES para palpites recém-inseridos (apenas se não existir cobrança vinculada)
...
-- 4) Verificação (revise antes do COMMIT)
SELECT 'RECAP' AS msg;
SELECT COUNT(*) AS palpites_inseridos
FROM palpites p
WHERE p.rodada = @rodada AND p.campeonato_id = @campeonato_id AND p.id_usuario IN (SELECT usuario_id FROM tmp_users);
...
-- COMMIT quando tiver revisado os resultados acima
-- COMMIT;

-- Se quiser reverter tudo (antes de COMMIT), use:
-- ROLLBACK;

-- Observações:
-- - O script não altera saldos nem gera lançamentos em extrato_movimentacao.
-- - Se quiser marcar palpites como 'pago' e criar cobranças já com status 'PAGO' (simulação de pagamento), eu adapto o script.
-- - Ajuste @users_csv, @rodada, @campeonato_id, @valor_palpite conforme necessário.
