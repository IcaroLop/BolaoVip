...
(async () => {
  ...
  try {
    ...
    for (let i = 0; i < palpites.length; i++) {
      ...
      const resultado = await connection.query(
        `INSERT INTO palpites 
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [19, 3, 10, 2, palpite.jogo_id, palpite.gols_casa, palpite.gols_fora]
      );
    ...
    const [verificacao] = await connection.query(
      'SELECT COUNT(*) as total FROM palpites WHERE id_usuario = 19 AND rodada = 2'
    );
    console.log(`\n📊 Verificação: ${verificacao[0].total} palpites para Cassiano Fortes na rodada 2`);
