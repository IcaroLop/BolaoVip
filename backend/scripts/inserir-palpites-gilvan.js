...
const palpites = [
  { jogo_id: 43725, gols_casa: 3, gols_fora: 0 },  // Flamengo 3 x 0 Internacional
...
(async () => {
  ...
  try {
    ...
    console.log('🔄 Iniciando inserção de palpites para Gilvan Ferreira da Silva...\n');
    ...
    for (let i = 0; i < palpites.length; i++) {
      ...
      await connection.query(
        `INSERT INTO palpites 
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [22, 3, 10, 2, palpite.jogo_id, palpite.gols_casa, palpite.gols_fora]
      );
    ...
    console.log('\n✅ Todos os 6 palpites de Gilvan foram inseridos com sucesso!');
    
    // Verificar inserção
    const [verificacao] = await connection.query(
      'SELECT COUNT(*) as total FROM palpites WHERE id_usuario = 22 AND rodada = 2'
    );
    console.log(`\n📊 Verificação: ${verificacao[0].total} palpites para Gilvan Ferreira da Silva na rodada 2`);
